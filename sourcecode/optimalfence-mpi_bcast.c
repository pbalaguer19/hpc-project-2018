...
int main(int argc, char *argv[]) {
    ...
    // Recibimos los optimos parciales.
    TListaArboles optimos[size];
    if(MPI_Allgather(
        &Optimo, 1, TreeListType,
        optimos, 1, TreeListType,
        MPI_COMM_WORLD) != MPI_SUCCESS) {
            perror("Error gathering optimal.");
            exit(-1);
    }

    printf("Rank: %d, Optimo PARCIAL: Coste: %f y tala %d arboles\n", 
    rank, optimos[rank].Coste, optimos[rank].NumArboles);
    
    // A partir de los N (size) optimos encontrados, determinar cual es el
    // óptimo final. Para ello utilizo la operacion REDUCE, con una operación
    // definida como getMin
    MPI_Op getMinOp;
    MPI_Op_create((MPI_User_function *) getMin, 1, &getMinOp);
    TListaArboles optimoFinal;

	if(MPI_Reduce(&optimos[rank], &optimoFinal, 1, 
